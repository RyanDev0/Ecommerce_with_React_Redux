...
const CartComp = ({ cart }) => {
  ...
  return (
    <div className='my-10 flex flex-col md:flex-row items-center justify-between'>
      ...
      <div className='text-xl font-bold mt-4 md:mt-0'>${cart?.price} ({cart?.quantity})</div>
      <div
          onClick={() => dispatch(removeFromCart(cart?.id))}
          className='bg-red-500 text-white w-[80px] text-xl cursor-pointer rounded-md h-12 flex items-center justify-center mt-4 md:mt-0 p-2'
        >
          Remove
      </div>
    </div>
